...
//    GRA_MYP_8003("MyG+ 오픈소스라이센스", "MYP-8003", HttpRequest.METHOD_POST, "/graapi/myp/openLicense.do", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    ...
    GRA_GNS_1006("My차고 + 렌트/리스 실 운행자 신청", "GNS-1006", HttpRequest.METHOD_POST, "/graapi/gns/myRentRegistration.do", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    GRA_GNS_1007("My차고 + 렌트/리스 실 운행자 내역", "GNS-1007", HttpRequest.METHOD_POST, "/graapi/gns/myRentDetail.do", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    ...
    GRA_GNS_1011("My차고 + 렌트/리스 실 운행자 확인", "GNS-1011", HttpRequest.METHOD_POST, "/graapi/gns/myRentOwnerConfirm.do", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    GRA_GNS_1012("My차고 + 렌트/리스 실 운행자 상세", "GNS-1012", HttpRequest.METHOD_POST, "/graapi/gns/myRentOwnerDetail.do", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    ...
    //DEVELOPERS API에서는 ifCd를 url 파라미터 형태로 사용 //todo 2021-01-11 GCS 통신 임시로 제거
    DEVELOPERS_DTC("고장 코드 조회", "carId", HttpRequest.METHOD_GET, "test/car/status/%s/dtc", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_REPLACEMENTS("소모품 교환 정보 조회", "carId", HttpRequest.METHOD_GET, "test/car/care/%s/replacements", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_TARGET("안전운전점수 가입여부 조회", "carId", HttpRequest.METHOD_GET, "test/car/model/%s/ubi/target", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_DETAIL("안전운전점수 상세 조회", "carId", HttpRequest.METHOD_GET, "test/car/model/%s/ubi/detail", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_CARLIST("내 차량 리스트 조회", "", HttpRequest.METHOD_GET, "test/car/profile/carlist", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_DTE("주행 가능 거리 조회", "carId", HttpRequest.METHOD_GET, "test/car/status/%s/dte", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_ODOMETER("누적 운행 거리 조회", "carId", HttpRequest.METHOD_GET, "test/car/status/%s/odometer", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_PARKLOCATION("최종 주차 위치 조회", "carId", HttpRequest.METHOD_GET, "test/car/status/%s/parklocation", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_DISTANCE("일별 운행 거리 조회", "carId", HttpRequest.METHOD_GET, "test/car/trip/%s/distance", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_CAR_CHECK("GCS 차량 목록 조회(대상 차량 존재 유무 확인)", "", HttpRequest.METHOD_GET, "test/user/cars", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_CAR_ID("CAR ID 조회(대상 차량 등록 여부 확인) (저장 시에도 요청 및 사용)", "userId", HttpRequest.METHOD_GET, "test/profile/users/%s/cars", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
    DEVELOPERS_CAR_CONNECT("GCS 차량 연결(등록)", "userId", HttpRequest.METHOD_POST, "test/profile/users/%s/cars", HttpRequest.CONTENT_TYPE_JSON, HttpRequest.CHARSET_UTF8),
