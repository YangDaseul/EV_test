...
import com.genesis.apps.databinding.ViewPlaymapBinding;
...
@AndroidEntryPoint
public class ViewPlayMap extends View {
    ...
    private ViewPlaymapBinding binding;

    public ViewPlayMap(Context context,MapViewModel mapViewModel) {
        super(context);
        binding = ViewPlaymapBinding.inflate((LayoutInflater)context.getSystemService(Context.LAYOUT_INFLATER_SERVICE));
        //       binding = DataBindingUtil.inflate((LayoutInflater)context.getSystemService(Context.LAYOUT_INFLATER_SERVICE), R.layout.view_playmap, this, true);
    ...
    public void setObserver(LifecycleOwner owner){
        ...
        this.mapViewModel.getPlayMapGeoItem().observe(owner, new Observer<NetUIResponse<PlayMapGeoItem>>() {
            ...
            public void onChanged(NetUIResponse<PlayMapGeoItem> playMapGeoItemNetUIResponse) {
                Toast.makeText(activity, playMapGeoItemNetUIResponse.data.addrRoad, Toast.LENGTH_LONG).show();
            }
        });
    }


    private void initMap(double lat, double lon, int zoom){
        binding.vMap.initMap(lat, lon, zoom);

        binding.vMap.onMapInitListener(new PlayMapView.OnMapInitCallback() {
            @Override
        ...
        binding.vMap.onMapLongPressListener(new PlayMapView.OnMapLongPressCallback() {
            @Override
...
//                eventTextView.setText("지도 롱탭 : " + String.format("%.5f", playMapPoint.getLongitude()) + ", " + String.format("%.5f", playMapPoint.getLatitude()));
        ...
        binding.vMap.onMapSingleTapUpListener(new PlayMapView.OnMapSingleTapUpCallback() {
            @Override
...
//                eventTextView.setText("지도 탭 : " + String.format("%.5f", playMapPoint.getLongitude()) + ", " + String.format("%.5f", playMapPoint.getLatitude()));
        ...
        binding.vMap.onMapDoubleTapUpListener(new PlayMapView.OnMapDoubleTapUpCallback() {
            @Override
...
//                eventTextView.setText("지도 더블탭 : " + String.format("%.5f", playMapPoint.getLongitude()) + ", " + String.format("%.5f", playMapPoint.getLatitude()));
        ...
        binding.vMap.onZoomLevelChangeListener(new PlayMapView.OnZoomLevelChangeCallback() {
            @Override
        ...
        binding.vMap.onMapCenterChangeListener(new PlayMapView.OnMapCenterChangeCallback() {
            @Override
